...
const AppProvider = ({ children }) => {
  ...
  const register = async (userInput) => {
    setLoading();
    try {
      const post = await axios.post(`/auth/register`, {
        ...userInput,
      });

      // console.log(post) : contenuto in READ_POST.md

      dispatch({ type: REGISTER_USER_SUCCESS, payload: post.data.user.name });
      localStorage.setItem(
        "user",
        JSON.stringify({ name: post.data.user.name, token: post.data.token })
      );
    } catch (error) {
